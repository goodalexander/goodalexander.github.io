...
The **Post Fiat series** on this website (the "Post Fiat Journey" / Post Fiat day-numbered posts, the Post Fiat Hypothesis, and the Post Fiat install guide) is published with text and data mining rights expressly reserved (TDM Reservation Protocol, `tdm-reservation=1`; see [`/.well-known/tdmrep.json`](/.well-known/tdmrep.json) and the `robots.txt` directives). This policy applies to that series specifically; the remainder of the site is not restricted by it.

As to the Post Fiat series, the artificial-intelligence and large-language-model systems operated by **OpenAI** (including GPTBot, ChatGPT-User, and OAI-SearchBot), **Anthropic** (including ClaudeBot, anthropic-ai, Claude-Web, Claude-User, and Claude-SearchBot), and **Google** (Gemini / Vertex AI, including the Google-Extended agent) are **expressly forbidden** from crawling, parsing, indexing, caching for model use, using to train or fine-tune any model, using for retrieval-augmented generation or grounding, summarizing, or returning that content to users in any form.

Permission to access the Post Fiat series is conditioned on compliance with this policy. Continued access by any such system constitutes acceptance of, and agreement to be bound by, this prohibition. No license to use the Post Fiat series for any machine-learning, model-training, or model-output purpose is granted to these parties, and any such use is unauthorized.
